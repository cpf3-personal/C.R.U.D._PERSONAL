...
# Ejemplo: 'Nombre de Mi Libro de Sheets' o la URL completa
...
def load_data_from_sheets():
    ...
    """
    st.info("Conectando y leyendo datos de Google Sheets...")
    ...
    try:
        ...
        for ws in worksheets:
            # st.write(f"Leyendo hoja: **{ws.title}**") # Eliminado para UI más limpia
        ...
        if selected_sheet in sheet_data:
            ...
            with st.expander("Filtros de Búsqueda"):
                ...
                if selected_filter_columns:
                    cols = st.columns(len(selected_filter_columns))
                    for i, col in enumerate(selected_filter_columns):
                        with cols[i]:
                            search_term = st.text_input(f"Buscar en {col}", key=f"filter_{selected_sheet}_{col}")
                            if search_term:
                                # (i?) es el flag de regex para "insensitive case"
                                filter_expressions.append(
                                    pl.col(col).fill_null("").str.contains(f"(?i){search_term}")
                                )
